...
const LoginPage = () => {
  ...
  const { addUserCreds, overideUser, saveUserEvent } = useContext(UserContext);
  ...
  const signup = async (d, newPhone) => {
    ...
    if (localStorage.getItem("ec_user_token") !== null) {
      ...
      jwt.verify(
        ...
        async (err, decoded) => {
          await saveUserEvent(decoded?.documentId, "signup");
          if (
            decoded &&
            decoded.hasOwnProperty("userCreds") &&
            typeof decoded["userCreds"] === "object"
          ) {
            console.log("has userCreds");
            await overideUser(d);
          } else {
            console.log("no userCreds availble");
            await addUserCreds(d);
          }
        }
      );
    } else {
  ...
  const login = (d) => {
    ...
    if (localStorage.getItem("ec_user_token") !== null) {
      ...
      jwt.verify(
        ...
        "d6d82b79-5226-454c-a36d-17bc13bcd6f2",
        async (err, decoded) => {
          await saveUserEvent(decoded?.documentId, "login");
        }
      );
    }
    // setErrors(false);
